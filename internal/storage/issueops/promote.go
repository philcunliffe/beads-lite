...
func PromoteFromEphemeralInTx(ctx context.Context, regularTx, ignoredTx *sql.Tx, id string, actor string) error {
	...
	if _, err := regularTx.ExecContext(ctx,
		insertSelectIgnoreSQL("labels", "issue_id, label", "SELECT issue_id, label FROM wisp_labels WHERE issue_id = ?"),
		id); err != nil {
		log.Printf("promote %s: failed to copy labels: %v", id, err)
	}

	if _, err := regularTx.ExecContext(ctx,
		insertSelectIgnoreSQL("dependencies", "issue_id, depends_on_id, type, created_at, created_by, metadata, thread_id", "SELECT issue_id, depends_on_id, type, created_at, created_by, metadata, thread_id FROM wisp_dependencies WHERE issue_id = ?"),
		id); err != nil {
		log.Printf("promote %s: failed to copy dependencies: %v", id, err)
	}

	if _, err := regularTx.ExecContext(ctx,
		insertSelectIgnoreSQL("events", "issue_id, event_type, actor, old_value, new_value, comment, created_at", "SELECT issue_id, event_type, actor, old_value, new_value, comment, created_at FROM wisp_events WHERE issue_id = ?"),
		id); err != nil {
		log.Printf("promote %s: failed to copy events: %v", id, err)
	}

	if _, err := regularTx.ExecContext(ctx,
		insertSelectIgnoreSQL("comments", "issue_id, author, text, created_at", "SELECT issue_id, author, text, created_at FROM wisp_comments WHERE issue_id = ?"),
		id); err != nil {
